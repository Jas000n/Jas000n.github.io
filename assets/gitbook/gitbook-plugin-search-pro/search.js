require(["gitbook", "jquery"], function (gitbook, $) {
    ...
    for (var page in INDEX_DATA) {
      if (
        ...
        results.push({
          ...
          body: INDEX_DATA[page].body
            .substr(Math.max(0, index - 50), MAX_DESCRIPTION_SIZE)
            .replace(
  ...
  window.addEventListener(
    ...
    function (e) {
      if (
        e.target.tagName === "A" &&
        e.target.getAttribute("data-need-reload")
      ) {
        setTimeout(function () {
          location.reload();
        }, 100);
